...
export default function NewsSection() {
  ...
  return (
    <section className="bg-[#292F36] text-white py-12 px-6">
      <div className="max-w-7xl mx-auto bg-[#1f242b] rounded-xl shadow-xl p-8">
        <h2 className="text-4xl font-extrabold text-[#4ECDC4] mb-10 text-center">
          NEWS
        </h2>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
          ...
          <div className="flex flex-col gap-4 max-h-[500px] overflow-y-auto pr-2 
            scrollbar-thin scrollbar-thumb-[#4ECDC4]/60 scrollbar-track-[#292F36]">
            {rightNews.map((item) => (
              <Link
                to={`/news/${item.id}`}
                key={item.id}
